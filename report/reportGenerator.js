...
function isHolidayOrLeave(holidays, leaves, employee, date) {
    ...
    for (leave of leaves) {
        var lsDate = moment(leave.date1)
            .tz("Asia/Colombo")
            .startOf("day");
        var leDate;

        if (leave.date2) {
            leDate = moment(leave.date2)
                .tz("Asia/Colombo")
                .endOf("day");
        } else {
            leDate = lsDate.clone();
        ...
        if (
            ...
            date.isSameOrAfter(lsDate) &&
            date.isSameOrBefore(leDate)
        ) {
            isHoL = true;
            break;
        }
...
async function generateReport() {
    var logs = await Log.find({ isArchieved: { $ne: true } })
        .populate("employee")
        .sort({ start: 1 });
        ...
        if (current_logs) {
            for (log of current_logs) {
                if (log.employee) {
                    ...
                    report += `${key},${log.employee.first_name},${start.format(
                        "HH:mm"
                    )},${end.format("HH:mm")},${total},${HoL}\n`;
                }
            }
        }
